...
default_ws="common-lisp lisp repo/common-lisp repo/lisp cl"

if [ -z $DWIM_WORKSPACE ] ; then

  for d in ${default_ws} ; do
      if [[ -e "$HOME/$d" ]] ; then
          DWIM_WORKSPACE="$HOME/$d"
          break
      fi
  ...
  if [ -z $DWIM_WORKSPACE ] ; then
      printf "\n\nEnvironment variable DWIM_WORKSPACE is not defined and I couldn't\nfind any of these directories (relative to your \$HOME):\n"
      printf "\n%s\n\n" "$default_ws"
      exit 1
  else
      echo "Using workspace $DWIM_WORKSPACE"
  fi

fi


LISP=${DWIM_WORKSPACE}/sbcl/run-sbcl.sh
LISP=`readlink -f ${LISP}`

if [ -z $LISP ] ; then
  echo
  echo "Running whatever SBCL is in the PATH, because there was no SBCL directory found in the workspace"
  echo
  LISP=sbcl
fi
...
(defun find-all-systems-with-substring (name-target)
...
(defun collect-to-be-loaded-systems-for-development-build (name-target)
  ...
  (let ((target-systems (find-all-systems-with-substring name-target))
