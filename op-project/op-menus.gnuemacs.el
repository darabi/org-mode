...
;--------------------
  ...
  (define-key op-mode-menu-bar-map [OUTLINE separator-toggle]
    '("--")) ; ------------------------------------------------------------

  (define-key op-mode-menu-bar-map [OUTLINE outline-toggle-children]
...
(define-key op-mode-menu-bar-map [OP-CONV op-text-extractor]
  '("xfig-Text in extra Fenster importieren" . xfig-importer))
(define-key op-mode-menu-bar-map [OP-CONV op-text-extractor]
  '("xfig-Text von Cursorposition extrahieren" . xfig-text-extractor))
(define-key op-mode-menu-bar-map [OP-CONV separator-others]
  '("--")) ; ------------------------------------------------------------
(define-key op-mode-menu-bar-map [OP-CONV csv-importer]
  '("csv-File in extra Fenster importieren" . csv-importer))
  '("--")) ; ------------------------------------------------------------
(define-key op-mode-menu-bar-map [OP-CONV mpx-text-extractor-1]
  '("MPX-File importieren" . mpx-import))
(define-key op-mode-menu-bar-map [OP-CONV mpx-text-extractor-1]
...
(define-key op-mode-menu-bar-map [OP-CONV separator-others]
  '("--")) ; ------------------------------------------------------------
(define-key op-mode-menu-bar-map [OP-CONV previous-dia-tag]
  '("Backward to previous <dia id=...> tag" . previous-dia-tag))
(define-key op-mode-menu-bar-map [OP-CONV next-dia-tag]
  '("Forward to next <dia id=...> tag" . next-dia-tag))
(define-key op-mode-menu-bar-map [OP-CONV dia-ID-synchronisator]
  '("Prepare Dia XML synchronisation " . dia-ID-synchronisator))
(define-key op-mode-menu-bar-map [OP-CONV dia-inserter-from-file]
  '("Import all new Dia XML text tags as headline from cursor position" . dia-inserter-from-file))
(define-key op-mode-menu-bar-map [OP-CONV dia-inserter]
  '("Import all new Dia XML text tags as headline" . dia-inserter))
...
(define-key op-mode-menu-bar-map [OP-MISC fill-paragraph]
  '("Fill Paragraph in Region" . op-fill-paragraph-region))
...
(define-key op-mode-menu-bar-map [OP-HEADINGS set-show-ausblend-all]
  '("Set Show All Toggles In Region"  . set-show-ausblend-all))
(define-key op-mode-menu-bar-map [OP-HEADINGS ausblend-1]
  '("Toggle *E[rledigtes] / *F[ällt]W[eg] in region" . ausblend-1))
(define-key op-mode-menu-bar-map [OP-HEADINGS ausblend-2]
  '("Toggle *S[tändiges] / *W[iederkehrendes] in region" . ausblend-2))
(define-key op-mode-menu-bar-map [OP-HEADINGS ausblend-3]
  '("Toggle *V[erschobenes] in region " . ausblend-3))
(define-key op-mode-menu-bar-map [OP-HEADINGS ausblend-4]
  '("Toggle *I[n]B[earbeitung] in region" . ausblend-4))
(define-key op-mode-menu-bar-map [OP-HEADINGS ausblend-6]
  '("Toggle *T[erminabsprachen] in region" . ausblend-6))
(define-key op-mode-menu-bar-map [OP-HEADINGS ausblend-5]
  '("Toggle *R[essourcen] in region" . ausblend-5))
(define-key op-mode-menu-bar-map [OP-HEADINGS nille]
  '("Markierung   *D   für  D[ringendes] \t(ohne Funktion)" . nil))
...
(define-key op-mode-menu-bar-map [OP-HEADINGS outline-forward-jump-same-level]
  '("Forward One Level Back" . outline-forward-jump-same-level))
(define-key op-mode-menu-bar-map [OP-HEADINGS outline-backward-jump-same-level]
  '("Backward One Level Back" . outline-backward-jump-same-level))
...
(define-key op-mode-menu-bar-map [OP-SHOW hide-sublevels-hide]
  '("Sublevels Till Level [ARG(=C-u)] //on region/on headline" . hide-sublevels-hide))
